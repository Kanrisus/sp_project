...
#AutoIt3Wrapper_Outfile=exe\run_spotify_1.Exe
...
Global $PathToControl = _PathFull("controller.exe")
Global $bExit= not False

Func run_controller()
	If Not ProcessExists("controller.exe") Then
		run ($PathToControl)
	EndIf
EndFunc
...
Func _run_spotify()
   ...
   $spotify= _PathFull(@AppDataDir & "\Spotify\Spotify.exe")
   $pid=run ($spotify)
